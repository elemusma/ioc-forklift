...
define( 'HASHBAR_WPNB_VERSION', '1.2.7');
...
function hashbar_wpnb_tablecreate(){
    ...
    if('1.2.4' <= $vesion){
        add_action('admin_notices','hashbar_wpnb_update_notice');
    }
...
function hashbar_wpnb_update_notice(){

    $user_id            = get_current_user_id();
    $user_closed_status = get_user_meta($user_id,'hthb_notice_dismissed',true);
    if($user_closed_status && 'true' == $user_closed_status ) return;

    ?>
        <div class="notice hthb-notice">
            <h3>Heads Up!</h3>
            <p>Thank you for keeping the <strong>"HashBar – WordPress Notification Bar"</strong> plugin up to date. In the most recent version, we have made a lot of significant changes to make the plugin more user friendly. The changes <a target="_blank" href="https://wordpress.org/plugins/hashbar-wp-notification-bar/#developers">can be viewed here</a>.</p><p>If you encounter any sort of issues after updating the plugin, <strong>Please clear your browser cache/cookies</strong> and <strong>also clear the Cache from your caching Plugin</strong>. If the problem still exists, please <a target="_blank" href="<?php echo esc_url( 'https://hasthemes.com/contact-us' ); ?>">Contact Hs Here</a>. We will provide you priority support and fix the issue in a quick time.</p>

            <a class="hthb-dismissable" href="<?php echo esc_url( add_query_arg('hthb-notice-dismissed','') ); ?>"></a>
        </div>
        <style type="text/css">
            .hthb-notice.notice{
            ...
            .hthb-notice.notice h3{
                margin-top: 10px;
            }
            .hthb-notice.notice p {
                font-size: 14px;
                max-width: 1020px;
            }
            .hthb-notice a.hthb-dismissable{
                position: absolute;
                right: 10px;
                top: 10px;
                text-decoration: none;
            }
            .hthb-notice a.hthb-dismissable::before{
                background: 0 0;
                color: #787c82;
                content: "\f153";
                display: block;
            ...
            .hthb-notice a.hthb-dismissable:hover::before{
                color: red;
            }
        </style>
    <?php
...
function  hashbar_wpnb_enqueue_scripts(){
    ...
    // enqueue js
    wp_enqueue_script( 'hashbar-frontend', HASHBAR_WPNB_URI.'/assets/js/frontend.js', array('jquery'),time(), false);
    wp_enqueue_script( 'hashbar-analytics', HASHBAR_WPNB_URI.'/assets/js/analytics.js', array('jquery'), time(), true );
...
function  hashbar_wpnb_admin_enqueue_scripts(){
    ...
    if((get_post_type() == 'wphash_ntf_bar' && isset($_GET['action']) && $_GET['action'] == 'edit') || (isset($_GET['post_type']) && $_GET['post_type'] == 'wphash_ntf_bar'))
    {
        ...
        wp_localize_script( 'hashbar-admin-js', 'hashbar_admin', $hashbar_localize_data);
    }
