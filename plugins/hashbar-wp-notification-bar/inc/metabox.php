...
if( !function_exists('hashbar_custom_date_field_callback') ){
	function hashbar_custom_date_field_callback( $args ){
		$post_id     = get_the_id();
		$field_value = get_post_meta($post_id, '_wphash_notification_schedule_datetime', true);
		?>
		<input type="text" name="_wphash_[_wphash_notification_schedule_datetime]" value="<?php echo esc_attr($field_value); ?>" data-depend-id="_wphash_notification_schedule_datetime" id="hthb_datepicker">
		<div class="clear"></div>
	...
	<script>
		;( function ( $ ) {
		    ...
		    $(document).ready(function(){
    		    $( "#hthb_datepicker" ).datetimepicker({
    		    	controlType: 'select',
    	    		oneLine: true,
    	    		dateFormat: 'mm/dd/yy',
    	    		timeFormat: 'hh:mm tt',
